...
module.exports = {
  run: async (hodnoty) => {
    ...
    ctx.fillStyle = color.dark_gray;
    ctx.fillRect(
      vyska / 2 + polomer + vzdalenost_od_avataru,
      vyska / 2 - sirkapole - vzdalenost1,
      sirka - (vyska / 2 + polomer + vzdalenost_od_avataru) - 40,
      sirkapole
    );
    ctx.fill();
    ...
    ctx.strokeText(
      stav,
      vyska / 2 + polomer + vzdalenost_od_avataru + text_mensi_o,
      110
    );
    ...
    ctx.fillText(
      user_username,
      vyska / 2 + polomer + vzdalenost_od_avataru + text_mensi_o,
      vyska / 2 - vzdalenost1 - text_mensi_o
    );
    ...
    ctx.fillText(
      ...
      vyska / 2 + polomer + vzdalenost_od_avataru + text_mensi_o,
      vyska / 2 + sirkapole + vzdalenost1 - text_mensi_o
    );
    ...
    ctx.font = `${sirkapole - velikost_textu}px Arial Black`;
    ctx.fillText(
      `${verze_pro_datum[stav_barva]}: ` + datum,
      vyska / 2 + polomer + text_mensi_o - 60,
      vyska - 42
    );
    ...
    const avatar = await loadImage(
      target.displayAvatarURL({ format: "jpg", size: rozliseni })
    );
    ctx.drawImage(
      avatar,
      vyska / 2 - polomer - sirkacary,
      vyska / 2 - polomer - sirkacary,
      256,
      256
    );
